...
export default function EditProduct() {
    ...
    useEffect(() => {
        try {
            fetch(`http://127.0.0.1:8000/api/factureGet/${id}`)
                .then(res => res.json())
                .then((data) => {
                    setDate(data[0].Date);
                    setClient(data[0].Id_Client);
                    setSeller(data[0].Id_Seller);
                    setPayment(data[0].Payment);
                    setReste(data[0].Reste);
                    setSold(data[0].Solde);
                    setType(data[0].Type);
                    setUpDate(data[0].Update_Date);
                    setUpTime(data[0].Update_Time);
                }
                )
        }
        catch (err) { }
    ...
    async function Submit(e) {
        ...
        try {
            ...
            await axios.post(`http://127.0.0.1:8000/api/factureUp/${id}`, {
                date,
                client,
                seller,
                payment,
